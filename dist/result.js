...
/***/ function(module, exports, __webpack_require__) {
	...
	new _field.Field(document.body, 60);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Field = function () {
	  function Field(elem) {
	    ...
	    var size = arguments.length <= 1 || arguments[1] === undefined ? 40 : arguments[1];
	    ...
	    for (var i = 1; i <= 10; i++) {
	      var x = size * i;
	      var y = size * i;
	      this.circles.push(new _circle.Circle({ x: x, y: y, size: size, field: elem }));
	    }
	
	    this.elem.addEventListener('dblclick', function (event) {
	      if (event.target === _this.elem) {
	        var check = _this.circles.every(function (circle) {
	          return _this.countDistance(circle, { x: event.pageX, y: event.pageY }) > size;
	        });
	        if (check) _this.circles.push(new _circle.Circle({ x: event.pageX, y: event.pageY, size: size, field: elem }));
	        return;
	      }
	      var circleForDelete = _this.circles.find(function (current) {
	        return current.node === event.target;
	      });
	      if (circleForDelete) _this.deleteCircle(circleForDelete);
	    });
	
	    this.elem.addEventListener('movement', function (event) {
	      var movedCircle = _this.circles.find(function (circle) {
	        return circle.x === event.detail.x && circle.y === event.detail.y;
	      });
	      if (!movedCircle) return;
	
	      var sibling = _this.checkSiblings(movedCircle);
	      while (sibling) {
	        movedCircle.moveable = false;
	        var mergedCircle = _this.mergeCircles(sibling, movedCircle);
	        _this.deleteCircle(sibling);
	        _this.deleteCircle(movedCircle);
	        sibling = _this.checkSiblings(mergedCircle);
	  ...
	  _createClass(Field, [{
	    key: 'checkSiblings',
	    value: function checkSiblings(circle) {
	      ...
	      try {
	        for (var _iterator = this.circles[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	          ...
	          var distance = this.countDistance(current, circle);
	    ...
	    key: 'countDistance',
	    value: function countDistance(_ref, _ref2) {
	      var x1 = _ref.x;
	      var y1 = _ref.y;
	      var x2 = _ref2.x;
	      var y2 = _ref2.y;
	
	      var side1 = Math.round(x1 - x2);
	      var side2 = Math.round(y1 - y2);
	      return Math.round(Math.sqrt(side1 * side1 + side2 * side2));
	    }
	  }, {
	    key: 'mergeCircles',
	    value: function mergeCircles(_ref3, _ref4) {
	      var x1 = _ref3.x;
	      var y1 = _ref3.y;
	      var color1 = _ref3.color;
	      var x2 = _ref4.x;
	      var y2 = _ref4.y;
	      var color2 = _ref4.color;
	
	      var middleX = Math.round((x1 + x2) / 2);
	      var middleY = Math.round((y1 + y2) / 2);
	      var c1 = parseInt(color1.slice(1), 16);
	      var c2 = parseInt(color2.slice(1), 16);
	      var middleColor = '#' + Math.floor((c1 + c2) / 2).toString(16);
	      var mergedCircle = new _circle.Circle({ x: middleX, y: middleY, color: middleColor, size: this.size, field: this.elem });
	      this.circles.push(mergedCircle);
	      return mergedCircle;
	    }
	  }, {
	    key: 'deleteCircle',
	    value: function deleteCircle(circle) {
	      circle.node.remove();
	      var idx = this.circles.findIndex(function (current) {
...
/***/ function(module, exports) {
	...
	var Circle = function Circle(options) {
	  ...
	  this.field.addEventListener('mouseup', function () {
	    ...
	    _this.node.style.zIndex = 'auto';
	  });
	  this.node.addEventListener('mousemove', function (event) {
	    if (!_this.moveable) return;
	    _this.node.style.left = event.pageX - _this.size / 2 + 'px';
	    _this.node.style.top = event.pageY - _this.size / 2 + 'px';
	    _this.x = event.pageX;
	    _this.y = event.pageY;
	    var eventMove = new CustomEvent('movement', { bubbles: true,
	      detail: Object.assign({}, _this) });
	    _this.node.dispatchEvent(eventMove);
	  });
	};
